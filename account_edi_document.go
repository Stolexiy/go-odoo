...
type AccountEdiDocument struct {
	...
	BlockingLevel *Selection `xmlrpc:"blocking_level,omitempty"`
	CreateDate    *Time      `xmlrpc:"create_date,omitempty"`
	CreateUid     *Many2One  `xmlrpc:"create_uid,omitempty"`
	DisplayName   *String    `xmlrpc:"display_name,omitempty"`
	EdiContent    *String    `xmlrpc:"edi_content,omitempty"`
...
func (c *Client) CreateAccountEdiDocument(aed *AccountEdiDocument) (int64, error) {
	ids, err := c.CreateAccountEdiDocuments([]*AccountEdiDocument{aed})
	if err != nil {
		return -1, err
	}
	if len(ids) == 0 {
...
// CreateAccountEdiDocument creates a new account.edi.document model and returns its id.
func (c *Client) CreateAccountEdiDocuments(aeds []*AccountEdiDocument) ([]int64, error) {
	var vv []interface{}
	for _, v := range aeds {
		vv = append(vv, v)
	}
	return c.Create(AccountEdiDocumentModel, vv, nil)
}
...
func (c *Client) UpdateAccountEdiDocuments(ids []int64, aed *AccountEdiDocument) error {
	return c.Update(AccountEdiDocumentModel, ids, aed, nil)
}
...
func (c *Client) FindAccountEdiDocumentIds(criteria *Criteria, options *Options) ([]int64, error) {
	return c.Search(AccountEdiDocumentModel, criteria, options)
}
